    ...
    public class MapFunction : JmesPathFunction
    {
        ...
        public override void Validate(params JmesPathFunctionArgument[] args)
        {
            if (!args[0].IsExpressionType)
                throw new Exception(
                    $"Error: invalid-type, function {Name} expects its first argument to be an expression type.");

            var array = args[1].Token;
            if (array.Type != JTokenType.Array)
                throw new Exception(
                    $"Error: invalid-type, function {Name} expects its second argument to be an array.");
        }
